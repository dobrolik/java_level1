...
public class Map extends JPanel {
    ...
    void modeTwoPlayer(int y, int x) {
        ...
        repaint();
        if (checkWin(cymbal)) {
            gameOverWindow.setMessage(stepPlayer2 ? "Player2 WIN!!" : "Player1 WIN!!",this);
            return;
        }
        if (isFuelFull()) {
    ...
    private boolean checkWin(char sym) {
        for (int i = 0; i < fieldSizeY; i++) {
            for (int j = 0; j < fieldSizeX; j++) {
                if (checkLine(i, j, 0, 1,  sym)) return true;   // линия х
                if (checkLine(i, j, 1, 0,  sym)) return true;   // линия у
                if (checkLine(i, j, 1, 1,  sym)) return true;   // диагональ х у
                if (checkLine(i, j, -1, 1, sym)) return true;  // диагональ х -у
            }
        }
        return false;
